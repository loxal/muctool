...
docker run -d -p 80:1180 -p 443:1443 -v ~/srv/muctool/logs:/home/svc_usr/logs --env BUILD=$env:BUILD --env BUILDD=$env:BUILD_COUNTER --env MY_ENV_SINGLE=$buildCounter --label teamcity=buildNumber --label sans-backing_service --name $docker_image loxal/${docker_image}:${docker_tag}
