...
docker run -d -p 80:1180 -p 443:1443 -v ~/srv/muctool/logs:/home/svc_usr/logs --env BUILD_COUNTER=$env:BUILD_COUNTER --label buildCounter=$env:BUILD_COUNTER --label sans-backing_service --name $docker_image loxal/${docker_image}:${docker_tag}
