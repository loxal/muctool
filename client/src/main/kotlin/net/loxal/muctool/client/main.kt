    ...
    init {
        document.addEventListener("DOMContentLoaded", {
            ...
            if (localStorage["accessToken"] != null) {
                ...
                val accessToken =
                    window.location.search.substring(window.location.search.indexOf("=") + 1) // once again, Edge proofs, it's an oddball
...
//        }
    ...
    private fun validateHtml() {
        ...
        xhr.onload = {
            if (xhr.status.equals(200)) {
                val isUtf8Html =
                    xhr.responseText.contains("Used the HTML parser. Externally specified character encoding was UTF-8.")
                val isValidHtml = xhr.responseText.contains("<p class=\"success\">")
                val auditEntry = document.createElement("li") as HTMLLIElement
                auditEntry.title =
                    "$htmlValidatorUrl - ${xhr.status} - Parsed as UTF-8 HTML: $isUtf8Html - Valit HTML: $isValidHtml"
    ...
    private fun initContainer(containerText: String) {
        ...
        (document.getElementById("main") as HTMLDivElement).innerHTML = previousPageContent
        applySiteProperties()
    ...
    private fun applySiteProperties() {
        ...
        xhr.onload = {
            val versionContainer = document.getElementById("title") as HTMLAnchorElement
            if (xhr.status.equals(200)) {
    ...
    private fun fetchUser(accessToken: String?) {
        ...
        xhr.onload = {
            if (xhr.status.equals(200)) {
                val user = JSON.parse<Client.OAuth2User>(xhr.responseText)
